...
function setup() {
  ...
  startButton.position(width / 2 - 50, height / 2 + 120);
...
function drawStartScreen() {
  ...
  textSize(50);
  text('星際射擊遊戲', width / 2, height / 2 - 180);
  
  textSize(20);
  fill(200);
  text('使用滑鼠瞄準並發射，打中星星得分', width / 2, height / 2 - 120);

  textAlign(LEFT, CENTER);
  textSize(16);
  let listX = width / 2 - 120;
  let startY = height / 2 - 80;
  let spacing = 25;

  fill(255);
  text('規則說明：', listX, startY);
  fill(0, 255, 255); text('● 青色圓形：凍結星星 10 秒', listX, startY + spacing);
  fill(255, 0, 255); text('■ 洋紅方形：無限彈藥 10 秒', listX, startY + spacing * 2);
  fill(255, 255, 0); text('▲ 黃色三角：增加時間 5 秒', listX, startY + spacing * 3);
  fill(255, 0, 0);   text('● 紅色炸彈：減少時間 3 秒', listX, startY + spacing * 4);
  fill(255);         text('⌨ 按下 [R] 鍵：手動重新裝填', listX, startY + spacing * 5);
}
